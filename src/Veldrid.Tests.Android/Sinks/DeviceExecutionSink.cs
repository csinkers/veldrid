    ...
    class DeviceExecutionSink : TestMessageSink
    {
        ...
        async Task MakeTestResultViewModel(ITestResultMessage testResult, TestState outcome)
        {
            if (!testCases.TryGetValue(testResult.TestCase, out TestCaseViewModel? testCase))
            {
                ...
                testCase = testCases
                    .FirstOrDefault(kvp =>
                        kvp.Key.UniqueID?.Equals(testResult.TestCase.UniqueID) ?? false
                    )
                    .Value;
                if (testCase == null)
                {
            ...
            TaskCompletionSource<TestResultViewModel> tcs = new(
            ...
            context.Post(
                _ =>
                {
                    ...
                    {
                        result.ErrorMessage = ExceptionUtility.CombineMessages(
                            (ITestFailed)testResult
                        );
                        result.ErrorStackTrace = ExceptionUtility.CombineStackTraces(
                            (ITestFailed)testResult
                        );
                    }
