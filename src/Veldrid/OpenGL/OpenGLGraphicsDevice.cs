...
internal sealed unsafe class OpenGLGraphicsDevice : GraphicsDevice
{
    ...
    DebugProc _debugMessageCallback;
    GCHandle _selfHandle;
    OpenGLExtensions _extensions;
    BackendInfoOpenGL _openglInfo;

    TextureSampleCount _maxColorTextureSamples;
    uint _maxTextureSize;
    uint _maxTexDepth;
    uint _maxTexArrayLayers;

    readonly StagingMemoryPool _stagingMemoryPool = new();
    ConcurrentQueue<ExecutionThreadWorkItem> _workItems;
    AutoResetEvent _workResetEvent;
    ...
    bool ManualSrgbBackbufferQuery()
    {
    ...
    static int GetDepthBits(PixelFormat value)
    {
        return value switch
        {
    ...
    int GetCount(OpenGLCommandList glCommandList)
    {
    ...
    sealed class ExecutionThread
    {
        readonly OpenGLGraphicsDevice _gd;
        readonly ConcurrentQueue<ExecutionThreadWorkItem> _workItems;
        ...
        ManualResetEvent RentResetEvent()
        {
        ...
        void ExecuteWorkItem(ref ExecutionThreadWorkItem workItem)
        {
        ...
        void ExecuteUnmapResource(MappableResource resource, uint subresource)
        {
            MappedResourceCacheKey key = new(resource, subresource);
        ...
        void EnqueueWork(ref ExecutionThreadWorkItem item)
        {
