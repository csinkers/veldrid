...
{
    static readonly FixedUtf8String s_name = "Veldrid-VkGraphicsDevice";
    static readonly Lazy<bool> s_isSupported = new(CheckIsSupported, isThreadSafe: true);
    ...
    readonly object _graphicsQueueLock = new();
    VkDebugReportCallbackEXT _debugCallbackHandle;
    bool _debugMarkerEnabled;
    vkDebugMarkerSetObjectNameEXT_t? _setObjectNameDelegate;
    vkCmdDebugMarkerBeginEXT_t? _markerBegin;
    vkCmdDebugMarkerEndEXT_t? _markerEnd;
    vkCmdDebugMarkerInsertEXT_t? _markerInsert;
    readonly ConcurrentDictionary<VkFormat, VkFilter> _filters = new();
    ...
    vkGetImageMemoryRequirements2_t? _getImageMemoryRequirements2;
    vkGetPhysicalDeviceProperties2_t? _getPhysicalDeviceProperties2;
    ...
    void CompleteFenceSubmission(in FenceSubmissionInfo fsi)
    {
    ...
    void ReturnSubmissionFence(VulkanFence fence)
    {
        _availableSubmissionFences.Enqueue(fence);
    }

    VulkanFence GetFreeSubmissionFence()
    {
    ...
    void SetResourceNameCore(DeviceResource resource, ReadOnlySpan<char> name)
    {
        switch (resource)
        {
    ...
    static IEnumerable<FixedUtf8String> GetSurfaceExtensions(HashSet<string> instanceExtensions)
    {
    ...
    IntPtr GetInstanceProcAddr(string name)
    {
        return VulkanUtil.GetInstanceProcAddr(_instance, name);
    }

    T? GetInstanceProcAddr<T>(string name)
    {
    ...
    [SkipLocalsInit]
    IntPtr GetDeviceProcAddr(ReadOnlySpan<char> name)
    {
